...
def useStandRegres():
    ...
    data_num, features_num = np.shape(lgX) # （63 x 4）矩阵
    lgX1 = np.mat(np.ones((data_num, features_num + 1))) # 创建一个矩阵 63 x 5 矩阵 各元素值为1
    lgX1[:, 1:5] = np.mat(lgX) # 将 lgX 63 x 4 矩阵 变为 63 x 5 矩阵 矩阵第一列的值为1
    ws = standRegres(lgX1, lgY) # 求解w值
...
def crossValidation(xArr, yArr, numVal = 10):
    ...
    """
    m = len(yArr)                                                                          #统计样本个数
    indexList = list(range(m))                                                             #生成样本标签索引值列表
    errorMat = np.zeros((numVal,30))                                                       #create error mat  10 x 30 列
    for i in range(numVal):                                                                #交叉验证numVal次
        ...
        wMat = ridgeTest(trainX, trainY)   # 30 x 4 矩阵系数                                             #获得30个不同lambda下的岭回归系数
        for k in range(30):                                                                #遍历所有的岭回归系数
            matTestX = np.mat(testX); matTrainX = np.mat(trainX)                        #训练集
